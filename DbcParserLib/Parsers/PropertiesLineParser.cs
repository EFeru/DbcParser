...
namespace DbcParserLib.Parsers
{
    internal class PropertiesLineParser : ILineParser
    {
        ...
        public bool TryParse(string line, IDbcBuilder builder, INextLineProvider nextLineProvider)
        {
            ...
            if (match.Success)
            {
                var isNumeric = !match.Groups[9].Value.StartsWith(ExtensionsAndHelpers.DoubleQuotes);
                var stringValue = match.Groups[9].Value.Replace(ExtensionsAndHelpers.DoubleQuotes, string.Empty);
