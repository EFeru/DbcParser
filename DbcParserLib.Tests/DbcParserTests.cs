...
namespace DbcParserLib.Tests
{
    public class DbcParserTests
    {
        ...
        public void CheckMessagePropertiesTest()
        {
            ...
            var targetMessage = parser.Messages.FirstOrDefault(x => x.ID == 309);
        ...
        public void CheckSignalPropertiesTest()
        {
            ...
            var targetMessage = parser.Messages.FirstOrDefault(x => x.ID == 1006);
            Assert.IsNotNull(targetMessage);
        ...
        public void CheckOtherSignalPropertiesTest()
        {
            ...
            var targetMessage = parser.Messages.FirstOrDefault(x => x.ID == 264);
            Assert.IsNotNull(targetMessage);
...
CM_ SG_ 1043 COUNTER_ALT ""only increments on change""; ";
            ...
            var parser = new DbcParser();
            parser.ReadFromString(dbc);

            Assert.AreEqual(3, parser.Messages.Count);
            Assert.AreEqual(0, parser.Nodes.Count);
            ...
            {
                var targetMessage = parser.Messages.FirstOrDefault(x => x.ID == messageIds[i]);
                Assert.IsNotNull(targetMessage);
